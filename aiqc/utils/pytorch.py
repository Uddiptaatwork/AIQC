...
def optimize(model, **hp):
    return torch.optim.Adamax(model.parameters(),lr=0.01)
...
def predict_regression(model, features):
    return model(features).detach().numpy()
...
def drop_invalidBatch(
    ...
    if (
        (last_batch_size == 1)
        and not allow_singleSample
        or last_batch_size != 1
        and enforce_sameSize
        and batch_size != last_batch_size
    ):
        batched_data = batched_data[:-1]
    elif last_batch_size == 1:
...
def flatten_uniColumn(tzr:object):
    return tzr.flatten() if (tzr.shape[-1]==1) else tzr
